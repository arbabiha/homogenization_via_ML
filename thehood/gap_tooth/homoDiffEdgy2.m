...
nPeriodsPatch=[1 1]
time=0:0.001:0.2; % for arbitrary times set intial and end time only, e.g, [0 0.2]
...
%}
if patches.EdgyEns 
   u0=repmat(permute(reshape(RandomIC(patches.x(:),patches.y(:)'),[nSubP(1),nPatch(1),nSubP(2),nPatch(2)]),[1,3,2,4]),1,1,1,1,mPeriod(1)*mPeriod(2));
   ...
   u0=permute(reshape(RandomIC(patches.x(:),patches.y(:)'),[nSubP(1),nPatch(1),nSubP(2),nPatch(2)]),[1,3,2,4]); 
